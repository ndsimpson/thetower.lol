...
def find_conflicting_records():
    ...
    active_sus = (
        ModerationRecord.objects.filter(moderation_type="sus", resolved_at__isnull=True)
        .select_related("known_player")
        .order_by("tower_id", "created_at")
    )

    active_bans = (
        ModerationRecord.objects.filter(moderation_type="ban", resolved_at__isnull=True)
        .select_related("known_player")
        .order_by("tower_id", "created_at")
    )
    ...
    for tower_id in sorted(conflicting_players):
        ...
        conflicts.append({"tower_id": tower_id, "player_name": player_name, "sus_records": sus_records, "ban_records": ban_records})

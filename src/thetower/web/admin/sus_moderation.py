...
def get_sus_moderation_raw_data():
    ...
    sus_records = ModerationRecord.objects.filter(
        moderation_type='sus',
        resolved_at__isnull=True  # Active records only
    ).select_related('known_player').order_by('-created_at')
    ...
    last_tournaments = (
        TourneyRow.objects
        .filter(player_id__in=tower_ids, result__public=True)
        .values('player_id')
        .annotate(last_tournament_date=Max('result__date'))
    )

    # Convert to dict for easy lookup
    last_tournament_lookup = {
        item['player_id']: item['last_tournament_date']
        for item in last_tournaments
    }
...
def render_sus_moderation_page():
    ...
    display_df['formatted_last_tournament'] = display_df['last_tournament_date'].apply(
        lambda x: x.strftime('%Y-%m-%d') if pd.notnull(x) else 'Never'
    )

    display_df['formatted_sus_created'] = display_df['sus_created_at'].apply(
        lambda x: x.strftime('%Y-%m-%d %H:%M') if pd.notnull(x) else ''
    )

    display_df['days_since_display'] = display_df['days_since_last_tournament'].apply(
        lambda x: f"{x} days" if pd.notnull(x) else 'Never'
    )

    # Select columns for display
    display_cols = [
        'clickable_player_id', 'player_name', 'formatted_last_tournament',
        'days_since_display', 'formatted_sus_created', 'sus_created_by'
    ]
    ...
    sortable_df = sortable_df[display_cols].rename(columns={
    ...
    st.dataframe(
        ...
        column_config={
            ...
            "Last Tournament": st.column_config.TextColumn(
                "Last Tournament",
                help="Date of last tournament participation"
            ),
            "Days Since": st.column_config.TextColumn(
                "Days Since",
                help="Days since last tournament"
            ),
            "Sus Created": st.column_config.TextColumn(
                "Sus Created",
                help="When the sus record was created"
            ),
            "Created By": st.column_config.TextColumn(
                "Created By",
                help="Who created the sus record"
            ),
            "Reason": st.column_config.TextColumn(
                "Reason",
                help="Reason for sus moderation"
            )
        }
        ...
        with col1:
            ...
            if days_filter != "All":
                ...
                if not filtered_df.empty:
                    ...
                    filtered_final = filtered_display[['tower_id', 'player_name', 'formatted_last_tournament', 'days_since_display']].rename(columns={
                        'tower_id': 'Tower ID',
                        'player_name': 'Player Name',
                        'formatted_last_tournament': 'Last Tournament',
                        'days_since_display': 'Days Since'
